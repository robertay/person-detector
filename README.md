...
track a specific person in an enclosed space
...
```bash
$ make bash
$ python person_detector/scripts/etl.py configs/examples/config_example.yml
```
...
```bash
python person_detector/scripts/evaluate.py configs/examples/config_example.yml
```
...
└── person_detector    <- Project source directory
